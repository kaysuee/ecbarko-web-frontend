...
export default function Bookings() {
  ...
  const openForm = (booking = null) => {
    if (booking && booking._id) {
      ...
      // Map passenger details from database format to form format
      ...
      // Map vehicle info from database format (single object) to form format (array)
      setVehicleDetails(booking.vehicleInfo && Object.keys(booking.vehicleInfo).length > 0 ? 
        [{
          ...
          vehicleSelect: (() => {
            // Find vehicle ID by matching vehicle name from vehicleCategories
          ...
          fareAmount: (() => {
            // Calculate vehicle fare from total payment minus passenger fares
            const totalPayment = booking.payment || 0;
            const passengerTotal = (booking.passengerDetails || []).reduce((sum, p) => sum + (p.fare || 0), 0);
            return Math.max(0, totalPayment - passengerTotal); // Ensure non-negative value
          })()
        }] : 
        [{ vehicleType: '', vehicleSelect: '', plateNumber: '', fareAmount: 0 }]
      );
  ...
  const confirmAdd = async () => {
    try {
      ...
      const bookingData = {
        ...
        departDate: new Date(selectedScheduleObj?.date || formData.departDate).toISOString(), // Ensure proper date format
